...
import { DropDownContainer, DropDownStyled, LoginButton, MobileBtnStyled, UserInfoStyled } from './styled'
    ...
    () => [
      {
        ...
        key: 'ticket',
        label: `${BNToFormat(auth?.user?.totalTicket) ?? 0}${t(
          BNToFormat(auth.user?.totalTicket) ?? 0 > 1 ? 'TICKETS' : 'TICKET'
        )}`,
        icon: <Ticket2Icon />
      },
      {
          ...
          {!auth?.isAuthenticated || !auth?.userProfile ? (
            ...
            <>
              <div className='h-100 p-relative d-flex j-content-center al-items-end flex-col'>
                <Typography.Text className='d-block text-right text-nowrap-1' title={auth?.userProfile?.userName}>
                  {auth?.userProfile?.userName}
                </Typography.Text>
                <Typography.Text className='d-block text-right text-nowrap-1 address' title={auth?.user?.publicAddress}>
                  {trimPublicAddress(auth?.user?.publicAddress, 5)}
                </Typography.Text>
              </div>
              <img
