...
import LocalMatchItem from './local-match-item'
...
const LocalMatchList: React.FC<Props> = ({ form }) => {
  const [betMatchList, setBetMatchList] = useState<any>([])
  const [loading, setLoading] = useState<boolean>(false)

  const getBetMatchList = useCallback(async () => {
    try {
      setLoading(true)
      const tournamentsList = await NSBService.getMatchList({
        langKey: LANG_KEY.JA
      })

      setBetMatchList(tournamentsList?.data)
      setTimeout(() => {
        setLoading(false)
      }, 1000)
    } catch (error) {
  ...
  return (
    <Spin spinning={loading} indicator={<SiteLoading />}>
      <MatchListContainerStyled>
        {_isEmpty(betMatchList) ? (
          <EmptyData />
        ) : (
          <CollapseComponent>
            {betMatchList?.map((panel: any, index: number) => {
              const customFormatDate = 'YYYY年MM月DD日'
              const keyIndex = String(index + 1)
              const { data, tournamentId, tournamentName, startDateTime, endDateTime } = panel || {}
                      ...
                      <Row align='middle' gutter={[24, 24]}>
                        {data?.map((match: any) => (
                          <Col span={24} key={match?.matchId}>
                            <LocalMatchItem form={form} match={match} />
                          </Col>
                        ))}
                      </Row>
                    </Panel>
