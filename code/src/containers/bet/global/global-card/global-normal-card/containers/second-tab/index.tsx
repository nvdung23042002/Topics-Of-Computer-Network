...
import { selectBet, setNormalCartItemsPaid } from '@/redux/bet/slice'
import { getNormalCartItemSecond } from '@/redux/bet/thunk'
...
const SecondTab = React.forwardRef((props: any, ref: any) => {
  ...
  const {
    normalCartItemsPaid,
    normalCartItemsPaidLoading,
    normalCartItemsPaidTotal,
    normalSumTicket,
    normalSumExpectedAmount
    ...
    ;(async () => {
      if (pagination.refresh) {
        if (isAuthenticated) {
          ...
          dispatch(setNormalCartItemsPaid([]))
        }
      }
    })()
  ...
  const fetchMoreData = () => {
    if (normalCartItemsPaid.length >= normalCartItemsPaidTotal) {
      setHasMore(false)
      return
    }
  ...
  return (
    <Spin spinning={normalCartItemsPaidLoading} indicator={<SiteLoading width={50} />}>
      <Divider />
      <BetCardPaidContentStyled>
        {_isEmpty(normalCartItemsPaid) ? (
          ...
          <CustomRowStyled>
            <ScrollParentCartItemsPaidStyled id='scroll-id-paid' ref={childRef}>
              <InfiniteScroll
                dataLength={normalCartItemsPaid.length || 0}
                next={fetchMoreData}
                hasMore={hasMore}
                loader={''}
              ...
              >
                <Row gutter={[20, 20]}>
                  {normalCartItemsPaid?.map((item: any) => {
                    return (
            ...
            <CustomSecondColStyled>
              <TotalBetStyled>
                ...
                <div className='total-tickets-owned'>
                  <Tooltip title={`${normalSumTicket?.toLocaleString('en-US') || 0} ${t('NUMBER_TICKETS')}`}>
                    <Text400Styled>
                      {normalSumTicket?.toLocaleString('en-US') || 0} {t('NUMBER_TICKETS')}
                    </Text400Styled>
                  </Tooltip>
                </div>
              </TotalBetStyled>
              <TotalMoneyPaymentStyled>
                ...
                <div className='value'>
                  <Tooltip title={`${normalSumExpectedAmount?.toLocaleString('en-US') || 0} ${t('CIRCLE')}`}>
                    <TextPrimary700Styled>
                      {normalSumExpectedAmount?.toLocaleString('en-US') || 0} {t('CIRCLE')}
                    </TextPrimary700Styled>
                  </Tooltip>
                </div>
              </TotalMoneyPaymentStyled>
