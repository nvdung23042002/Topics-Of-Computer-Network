...
const NormalBetCart: React.FC<Props> = ({ form }) => {
  const { t } = useTranslation('cart')
  const router = useRouter()
  const { activeNormalCartTab } = router.query || {}
  const { normalCartItems } = useAppSelector(selectBet) || {}
  const childRef = useRef<any>(null)
  const [currentTab, setCurrentTab] = useState<number>(1)

  useEffect(() => {
    setCurrentTab(Number(activeNormalCartTab) || 1)
  }, [])
  ...
  return (
    <BetCartStyled
      className={cx('col-bet-right', {
        'no-data': _isEmpty(normalCartItems)
      })}
      maxWidth={430}
      bgColor={true}
      padding={'21px 15px'}
...
export default memo(NormalBetCart)
