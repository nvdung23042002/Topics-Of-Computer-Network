...
export default function CancelComponent({ getOrderDetail, setCount }: IComponent) {
  ...
  const fetchData = async (query) => {
    setLoading(true)
    try {
      const response = await HistoryService.getShopOnlineHistory({ ...query, status: 'CANCELLED_TRANSFERRED' })
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'status',
      key: 'status',
      render: () => <TextStyled className='error'>{t('HISTORY.STATUS_FAIL_PAID')}</TextStyled>
    },
    {
