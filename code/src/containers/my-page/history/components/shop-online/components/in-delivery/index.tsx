...
export default function InDeliveryComponent({ getOrderDetail, setCount }: IComponent) {
  const { t } = useTranslation('history')

  // const { orderProgess, orderProgessLoading } = useAppSelector((state) => state.historyOrders)
  ...
  const fetchData = async (query) => {
    setLoading(true)
    try {
      const response = await HistoryService.getShopOnlineHistory({
        ...query,
        status: ['PROGRESS', 'DELIVERY', 'CANCELLED_TRANSFERRING'].join(',')
      })
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, { status }) => {
        ...
        switch (status) {
          case 'PROGRESS':
            text = t('HISTORY.STATUS_INPROGRESS')
            break
        ...
        return <TextStyled className='inprogess'>{text}</TextStyled>
      }
    },
    {
