...
const ModalConfirmBuyTicketByCreditCard = forwardRef<ModalRef, ModalProps>(
  ({ data = { amount: 0, paymentMethod: '' }, handleSubmit, priceOfTicket, ...props }, fwRef) => {
    ...
    return (
      <ModalStyled
        ...
        footer={null}
        title={
          <ContainerTitleModalInfo className='credit-card'>
            <TitleStyled className='title-modal'>{t('TITLE_PURCHASE_CONFIRM_MODAL')}</TitleStyled>
          </ContainerTitleModalInfo>
        }
        className='custom-modal-antd'
      >
        <ContainerInfo className='credit-card'>
          <DescriptionsStyled size={'small'} column={1} className='credit-card'>
          ...
          <MessageConfirm className='credit-card-success'>{t('CONFIRM_BUY_TICKET_MESSAGE')}</MessageConfirm>
...
export default ModalConfirmBuyTicketByCreditCard
